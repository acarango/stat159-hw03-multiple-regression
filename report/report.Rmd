...
* Firstly, we see that the most positive coefficient is for `r names(max(advertising$coefficients))` and the most negative coefficient is `r names(min(advertising$coefficients))`. This basic info leads us to believe that advertising budget should be directed more towards `r names(max(advertising$coefficients))` and it should be taken away from `r names(min(advertising$coefficients))`. 
